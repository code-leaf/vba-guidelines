...
提示された VBA コードを、**保守性・再利用性・パフォーマンス・可読性・事故防止**を最大化するようブラッシュアップしてください。  
出力はブラッシュアップ後の**完全な修正版コード**として提示すること。  
元の構造や意図を理解した上で、改善を行うこと。

## あなたの役割

20 年以上のキャリアを持つ VBA プログラマーとして、実務で長期運用できる高品質なコードを作成してください。
...
- **可読性 ＞ 処理速度 ＞ 省行数** を優先する
- **DRY 原則**: 重複を排除し共通化
- **単一責任**: 1 プロシージャ = 1 機能（50〜80 行以内を目安）
- **関心の分離**: データ処理・UI・I/O を明確に分ける
...
- ループカウンタは `i`, `j` など簡潔で OK
- Enum 以外で日本語変数名は禁止
- **Type は構造体として関連情報をまとめる場合のみ使用可**
...
ToggleAppState 関数を使い、画面更新・イベント・再計算をまとめて制御する。
False で最適化モード（高速化）、True で通常状態に戻す。
...
**Key は必ず意味が分かる複合キーとする（区切り文字は \_ を使用）**
...
※特別な理由（Excel 単体動作を優先する等）がない限り、参照設定を行う。
...
- [ ] Dictionary 使用時は複合キー（\_区切り）
- [ ] 配列経由でセルアクセス
- [ ] 書式・罫線は共通 Sub 化

---

以上の指示に従い、下記の VBA コードをブラッシュアップしてください。
